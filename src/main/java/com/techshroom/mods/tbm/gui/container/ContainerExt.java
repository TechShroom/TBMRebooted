...
public abstract class ContainerExt extends Container {
    ...
    public ContainerExt(IInventory inv, EntityPlayer player) {
        this(inv, SLOTS_PER_ROW_DEFAULT, player);
    }

    public ContainerExt(IInventory inv, int rowSize, EntityPlayer player) {
        this.numRows = inv.getSizeInventory() / (this.slotsPerRow = rowSize);
        if (this.numRows * this.slotsPerRow < inv.getSizeInventory()) {
    ...
    protected ContainerExt withBalancedSlots() {
        int slotCount = this.inventory.getSizeInventory();
        int slotIndex = 0;

        for (int row = 0; row < this.numRows && slotIndex < slotCount; ++row) {
            for (int col = 0; col < 9 && slotIndex < slotCount; ++col) {
                this.addSlotToContainer(new Slot(this.inventory, slotIndex,
                        8 + col * 18, 18 + row * 18));
                slotIndex++;
            }
        }
    ...
    protected ContainerExt withPlayerInventory(InventoryPlayer playerInv) {
        this.usingPlayerInventory = true;
    ...
    protected ItemStack shiftClick(EntityPlayer player, int slotIndex) {
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            int notPlayerSize = this.inventory.getSizeInventory();
